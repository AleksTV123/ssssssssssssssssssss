...
class MinecraftBot {
  ...
  private setupEventListeners(): void {
    ...
    this.bot.on('spawn', () => {
      ...
      // Start intervals for attack and repair
      this.startAttackInterval();
      this.startRepairInterval();
  ...
  // Start the interval for performing sweep attacks
  private startAttackInterval(): void {
    this.attackInterval = setInterval(() => {
      this.performSweepAttack();
    }, 1000);
  }

  // Start the interval for sending repair commands
  private startRepairInterval(): void {
    this.repairInterval = setInterval(() => {
      if (this.bot) {
        this.bot.chat('/repair all');
        this.log('repaired', 'system');
        this.lastAction = 'Repair command';
        this.status.lastAction = `Last repair: ${this.formatTime(new Date())}`;
        this.broadcastStatus();
      }
    }, 10000);
  }

  // Perform a sweep attack on nearby armor stands
  private performSweepAttack(): void {
    if (!this.bot) return;

    // Get the entities around the bot
    const entities = this.bot.entities;

    // Find the closest armor stand
    let target = null;
    ...
    if (target) {
      ...
      this.log('Bot performed a sweep attack on the armor stand.');
      this.lastAction = 'Attack';
      this.status.activity = 'Attacking';
      this.broadcastStatus();
    } else {
      this.log('No armor stand found in front of the bot.');
    }
  }
